...
{
  FEMSpace<vec_type,DIM>& sp = ele.femSpace();
...
void uiExperiment::updateDGGeometryCache(std::vector<EdgeCache<vec_type,DIM> >& edge_cache, u_int alg_acc)
{
  const u_int& n_edge = fem_space->n_DGElement();
  edge_cache.clear();
  ...
  for(u_int i = 0;the_dgele != end_dgele;++ the_dgele,++i){
    ...
    EdgeCache<vec_type,DIM>& ec = edge_cache[i];
    ec.idx = edge_idx;
