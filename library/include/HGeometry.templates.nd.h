...
void IrregularMesh<2, DOW>::regularize(bool renumerate)
{
  ...
  for (;the_ele != end_ele;++ the_ele) {
    ...
    for (i = 0;i < the_ele->n_boundary;i ++) { /// ??
      ...
      if (tools.isRefined(side)) { 
        tools.setGeometryActive(*side.child[0]->vertex[1]); /// ???��?
  ...
  for (;the_ele != end_ele;++ the_ele) {
    ...
    for (i = 0;i < the_ele->n_boundary;i ++) { /// ???
      HGeometry<1, DOW>& side = *(h_element->boundary[i]);
      if (tools.isGeometryActive(side)) { /// ??????
        side.index = n_side ++;
      } else if (tools.isGeometryInactive(side)) { /// ?????
        HGeometry<0, DOW>& mp = *(side.child[0]->vertex[1]);
        if (tools.isGeometryActive(mp)) { /// ?????��?
          mp.index = n_node ++;
        }
        for (j = 0;j < side.n_child;++ j) { /// ?????????????
  ...
  for (;the_ele != end_ele;++ the_ele) {
    ...
    for (i = 0;i < the_ele->n_boundary;i ++) {
      ...
      if (tools.isGeometryInactive(bnd)) { // ??????????
...
void IrregularMesh<3, DOW>::regularize(bool renumerate)
{
  ...
  for (;the_ele != end_ele;++ the_ele) {
    ...
    for (int i = 0;i < h_element->n_boundary;++ i) { /// ??
  ...
  for (;the_ele != end_ele;++ the_ele) {
    ...
    for (int i = 0;i < the_ele->n_boundary;++ i) {
      ...
      if (tools.isGeometryIndexed(*bnd)) { // ?????active??
	/**
         * ????????????????????????��???????????��?????????????
         * ?��???????????????? refined_edge ?��?????????????
         * ???????k????? HGeometry<2, DOW>::n_boundary ?��???????
         * ?????????��??? k==HGeometry<2, DOW>::n_boundary??
         */
        int k = 0;
	...
	if (surface.index() != -1) continue; // ???????��???????????????

	if (k == bnd->n_boundary) { // ???????????
	  for (int l = 0;l < bnd->n_boundary;l ++) { // ?????????
	...
	tetra.boundary(k+4) = h_element->boundary[t[k+1]]->index;
      ...
      n_four_tetrahedron ++;
    } else if (n_twin_triangle_surface > 0) { // ??????????
...
template <>
void HGeometryTree<2, DOW>::readTriangle(const std::string& filename)
{
  ...
  HGeometry<0, DOW> * node_temp = new HGeometry<0, DOW>();
  HGeometry<1, DOW> * side_temp = new HGeometry<1, DOW>();
  is >> n_node >> temp >> attribute_flag >> marker_flag;
  Assert(temp == 2, ExcMeshData("triangle only support dim = 2"));
  std::vector<HGeometry<0, DOW> *> node(n_node);

  std::cout << "\treading the nodes data ..." << std::flush;
  for (i = 0;i < n_node;i ++) {
    node[i] = new HGeometry<0, DOW>();
    Assert(node[i] != NULL, ExcOutOfMemory());
    is >> j
  ...
  std::cout << "\treading the sides data ..." << std::flush;	
  for (i = 0;i < n_side;i ++) {
    side[i] = new HGeometry<1, DOW>();
    Assert(side[i] != NULL, ExcOutOfMemory());
    is >> l >> j >> k >> bm;
  ...
  std::cout << "\treading the elements data ..." << std::flush;	
  for (i = 0;i < n_element;i ++) {
    element[i] = new HGeometry<2, DOW>();
    Assert(element[i] != NULL, ExcOutOfMemory());
    is >> l >> j >> k >> l >> bm;
    ...
    for (p = 0; p < n_side; p++) {
        if (temp == 3)
            break;
        if ((side[p]->vertex[0] == node[j] && side[p]->vertex[1] == node[k]) 
        || (side[p]->vertex[1] == node[j] && side[p]->vertex[0] == node[k])){
            element[i]->boundary[2] = side[p];
            temp ++;
            continue;
        }             
        if ((side[p]->vertex[0] == node[j] && side[p]->vertex[1] == node[l])
        || (side[p]->vertex[1] == node[j] && side[p]->vertex[0] == node[l])){
            element[i]->boundary[1] = side[p];
            temp ++;
            continue;
        }
        if ((side[p]->vertex[0] == node[k] && side[p]->vertex[1] == node[l])
        || (side[p]->vertex[1] == node[k] && side[p]->vertex[0] == node[l])){
            element[i]->boundary[0] = side[p];
            temp ++;
            continue;
        }
        #if (DOW == 2)
          const Point<2>& p0 = *(dynamic_cast<Point<DOW> *>(node[j]));
          const Point<2>& p1 = *(dynamic_cast<Point<DOW> *>(node[k]));
          const Point<2>& p2 = *(dynamic_cast<Point<DOW> *>(node[l]));
          if ((p1[0] - p0[0])*(p2[1] - p0[1]) - (p1[1] - p0[1])*(p2[0] - p0[0]) < 0) {
              ...
              node[l] = node[k];
              node[k] = node_temp;
              side_temp = element[i]->boundary[2];
              element[i]->boundary[2] = element[i]->boundary[1];
              element[i]->boundary[1] = side_temp;
          }
        #endif
    }  
