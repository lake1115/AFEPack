...
cvaltype uiExperiment::get_u_hat(int n, int m)
{
  ...
  double M = dg_dof[2].size();
  double theta,phi;
  cvaltype u_hat = 0;
  double N = 1.0*n;
  for(int i = 0; i < M; i++){
    const Point<DIM> point = fem_space->dofInfo(dg_dof[2][i]).interp_point;
    ...
    cvaltype cout = u_re(dg_dof[2][i]) + I*u_im(dg_dof[2][i]);
  ...
  for(int n = 0; n <= order; n++){
      ...
      for (u_int i = 0; the_dgele != end_dgele; ++ the_dgele,++i)
	{
	  ...
	  for(int l=0;l<n_q_pnt;l++){
	    ...
	    cvaltype Y = boost::math::spherical_harmonic(n,m,theta,phi);
	    for(int j=0;j<n_dgele_dof;j++){
	      vec_u.coeffRef(dgele_dof[j]) += Jxw*bas_val[j][l]*std::conj(Y);
	      
	      vec_v.coeffRef(dgele_dof[j]) += Jxw*bas_val[j][l]*Y;
	    }
	  }
	}
	...
	for(Eigen::SparseVector<cvaltype>::InnerIterator it_v(vec_v); it_v; ++it_v){
	  cvaltype cout = -1/R*Theta*it_u.value()*it_v.value();
...
void uiExperiment::adaptMesh()
{
  ...
  mesh_adaptor.tolerence() = 1.0e-3; /// 3.5e-3
...
void uiExperiment::getIndicator()
{
 ...
 // once more cycle for calculate indicator
  ...
  for(int i=0; the_ele!= end_ele; ++ the_ele, ++i)
	...
	B += 0.25*ec.es_list[j]*jump[bnd.index()];
      }
...
void uiExperiment::buildDGFEMSpace(int bmark)
{
  ...
  ////////////////////////////
  ///get dof in ball boundary
  if(bmark !=0){
    double M = 0;
    for(int i=0;i<fem_space->n_dof(); i++){
      if(fem_space->dofInfo(i).boundary_mark != bmark)
	continue;
      M++;
    }
    std::cout<<" dof of "<<bmark<<" = "<<M<<std::endl;
    dg_dof[bmark].resize(M);
    ...
    /// after refine mesh, push the point onto the ball
    for(int i = 0;i<M;i++){
      GeometryBM& geo = mesh.geometry(0,dg_dof[bmark][i]);
      Point<DIM>& p = mesh.point(geo.vertex(0));
      double z = sqrt(p[0]*p[0]+p[1]*p[1]+p[2]*p[2]);
      if(bmark == 1)
	{
	  p[0] = p[0]*r/z;
	  p[1] = p[1]*r/z;
