...
void uiExperiment::init()
{
  ...
  triangle_basis_function_y.reinit(triangle_template_dof);
  ...
  triangle_template_dof_u.readData("triangle.0.tmp_dof");
  triangle_basis_function_u.reinit(triangle_template_dof);
  triangle_basis_function_u.readData("triangle.0.bas_fun");
  ...
  twin_triangle_basis_function_y.reinit(twin_triangle_template_dof);
...
void uiExperiment::buildFEMSpace()
{
  ...
  u_int n_element = mesh_y.n_geometry(DIM);
  fem_space_y.element().resize(n_element);
  ...
  u_int n_element = mesh_u.n_geometry(DIM);
  fem_space_u.element().resize(n_element);
  for(int i=0; i<n_element; i++){
    // if vertex is 3, use triangle, else use twin triangle 
  ...
  const int& n_dof_y = fem_space_y.n_dof();
  stiff_matrix_y.resize(n_dof_y,n_dof_y);
  rhs_y.setZero(n_dof_y);
  solution_y.setZero(n_dof_y);
  triplets_y.clear();
  
  const int& n_dof_u = fem_space_u.n_dof();
  stiff_matrix_u.resize(n_dof_u,n_dof_u);
  rhs_u.setZero(n_dof_u);
  solution_u.setZero(n_dof_u);
  triplets_u.clear();
  
  std::cout << "Update Data Cache ...";
  updateGeometryCache(fem_space_u);
  updateGeometryCache(fem_space_y);
