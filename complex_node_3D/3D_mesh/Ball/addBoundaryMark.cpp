...
const double radius_sphere = 1.0;
...
int main(int argc, char* argv[])
{
  ...
  std::cout << "boundary points number: " << n_boundary_point << std::endl; 
  ...
  for(int i = 0;i < n_line;++ i){
    ...
    line_geo.boundaryMark() = 2;/// assume it is boundary
  ...
  for(int i = 0;i < n_tri;++ i){
    GeometryBM& tri_geo = mesh.geometry(2, i);
    if(tri_geo.boundaryMark() == 2)
      n_boundary_surface++;
  }
  std::cout << "boundary surface number: " << n_boundary_surface<<std::endl;
