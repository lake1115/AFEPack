...
template <class value_type, int DIM, int DOW=DIM, int TDIM=DIM, typename Number=double>
  ...
  void writeOpenDXData(const std::string& filename,
                       int flag = 0) const;
  /**
   * Write the data of the function in Femm data format by Hu Bin
   * 
   * @param filename the file name is same as the mesh file, which need to read .fem 
   */
  void writeFemmData(const std::string& filename);
