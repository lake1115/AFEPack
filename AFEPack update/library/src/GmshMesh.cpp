...
void GmshMesh::readData(const std::string& filename)
{
  ...
  for (i = 0, j = 0;i < n_geometry;i ++) {
    is >> k; // geometry number    1
    is >> gtype; // geometry type  15
    is >> n_tags; // number of tags;  2
    is >> k; // reading the tags, the first tag is the boundary mark   1
    for (l = 1;l < n_tags;++ l) is >> m;  // the second tag is the number of this kind of geometry, usually is useless
    GeometryBM geo;
    switch (gtype) {
    case POINT:
      geo.boundaryMark() = k; // geometry region(material marker)  1
      k = N_POINT_NODE;
      geo.vertex().resize(k);  // 1
	...
	geo.vertex(l) = ind[m];
      ...
      break;
    case TETRAHEDRON:
// I think tetrahedron's bmark is also useful ,but no geo to get
      // geo.boundaryMark() = k;
      //
